...
import io.github.mzanthem.spring.JclSpringContextUtil;
...
public class DynamicAspect implements IDynamicAspect{
	...
	//原来的class
	private static final String serviceName3 = "createStockService";
	 ...
	 */
	@Override
	public boolean execute(String jclName, String methodName, Class<?>[] parameterTypes, Object[] args) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, InstantiationException, NoSuchMethodException, SecurityException {
		...
		System.out.println(interfaces2);
		
		
		//获得class3
		Object service3 = JclSpringContextUtil.getBean(serviceName3);
		
		
		//根据类名获取Class对象  构造class0
		Class<?> clazz0 = JarClassManager.getLoadedClass(jclName, className0);
		//参数类型数组
		Class<?>[] parameterTypes0={interfaces1, interfaces2, CreateStockService.class};
		//根据参数类型获取相应的构造函数
		Constructor<?> constructor=clazz0.getConstructor(parameterTypes0);
		//参数数组
		Object[] parameters={instance1, instance2, service3};
		//根据获取的构造函数和参数，创建实例
